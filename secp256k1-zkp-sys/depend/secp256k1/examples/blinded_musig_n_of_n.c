...
int sign_and_verify(rustsecp256k1zkp_v0_8_1_context* ctx, const size_t _server_count) {
    ...
    const rustsecp256k1zkp_v0_8_1_musig_partial_sig **partial_sig_arg;
    ...
    pubkeys_ptr = malloc((server_count + 1) * sizeof(rustsecp256k1zkp_v0_8_1_pubkey*));

    pubnonces = malloc((server_count + 1) * sizeof(rustsecp256k1zkp_v0_8_1_musig_pubnonce*));

    partial_sig = malloc((server_count + 1) * sizeof(rustsecp256k1zkp_v0_8_1_musig_partial_sig));

    partial_sig_arg = malloc((server_count + 1) * sizeof(rustsecp256k1zkp_v0_8_1_musig_partial_sig*));
    ...
    if (!rustsecp256k1zkp_v0_8_1_musig_pubkey_agg(ctx, NULL, &aggregate_xonly_pubkey, &cache, pubkeys_ptr, server_count + 1)) {
        printf("fail\n");
    ...
    if (!rustsecp256k1zkp_v0_8_1_musig_nonce_agg(ctx, &agg_pubnonce, pubnonces, server_count + 1)) {
        printf("fail\n");
    ...
    if (!rustsecp256k1zkp_v0_8_1_musig_partial_sign(ctx, &partial_sig[0], &client_data.secnonce, &client_data.keypair, &cache, &session)) {
        printf("fail\n");
        printf("Client failed to sign message\n");
        goto cleanup;
    }   

    if (!rustsecp256k1zkp_v0_8_1_musig_partial_sig_verify(ctx, &partial_sig[0], &client_data.pubnonce, &client_data.pubkey, &cache, &session)) {
        printf("fail\n");
    ...
    if (!rustsecp256k1zkp_v0_8_1_musig_partial_sig_agg(ctx, agg_sig, &session, partial_sig_arg, server_count + 1)) {
        printf("fail\n");
        printf("Failed to aggregate partial signatures\n");
        goto cleanup;
    }

    if (!rustsecp256k1zkp_v0_8_1_schnorrsig_verify(ctx, agg_sig, msg, 32, &aggregate_xonly_pubkey)) {
