...
int main(void) {
    ...
    rustsecp256k1zkp_v0_8_1_context* ctx = rustsecp256k1zkp_v0_8_1_context_create(SECP256K1_CONTEXT_NONE);
    if (!fill_random(randomize, sizeof(randomize))) {
        printf("Failed to generate randomness\n");
        return 1;
    }
    /* Randomizing the context is recommended to protect against side-channel
     * leakage See `rustsecp256k1zkp_v0_8_1_context_randomize` in secp256k1.h for more
     * information about it. This should never fail. */
    return_val = rustsecp256k1zkp_v0_8_1_context_randomize(ctx, randomize);
    assert(return_val);
     ...
     * `rustsecp256k1zkp_v0_8_1_keypair_xonly_pub` supports returning the parity for
     * other use cases such as tests or verifying Taproot tweaks.
     * This should never fail with a valid context and public key. */
    return_val = rustsecp256k1zkp_v0_8_1_keypair_xonly_pub(ctx, &pubkey, NULL, &keypair);
    assert(return_val);

    /* Serialize the public key. Should always return 1 for a valid public key. */
    return_val = rustsecp256k1zkp_v0_8_1_xonly_pubkey_serialize(ctx, serialized_pubkey, &pubkey);
    assert(return_val);
     ...
     *
     * We use rustsecp256k1zkp_v0_8_1_tagged_sha256 to create this hash. This function expects
    ...
    return_val = rustsecp256k1zkp_v0_8_1_tagged_sha256(ctx, msg_hash, tag, sizeof(tag), msg, sizeof(msg));
    assert(return_val);
     ...
     *
     * We use the rustsecp256k1zkp_v0_8_1_schnorrsig_sign32 function that provides a simple
    ...
    return_val = rustsecp256k1zkp_v0_8_1_schnorrsig_sign32(ctx, signature, msg_hash, &keypair, auxiliary_rand);
    assert(return_val);
     ...
     * be parsed correctly */
    if (!rustsecp256k1zkp_v0_8_1_xonly_pubkey_parse(ctx, &pubkey, serialized_pubkey)) {
    ...
    is_signature_valid = rustsecp256k1zkp_v0_8_1_schnorrsig_verify(ctx, signature, msg_hash, 32, &pubkey);
    ...
    /* Bonus example: if all we need is signature verification (and no key
       ...
       rustsecp256k1zkp_v0_8_1_context_create(). We can simply use the static (i.e., global)
       context rustsecp256k1zkp_v0_8_1_context_static. See its description in
       include/secp256k1.h for details. */
    is_signature_valid2 = rustsecp256k1zkp_v0_8_1_schnorrsig_verify(rustsecp256k1zkp_v0_8_1_context_static,
