...
int test_sign_verify(rustsecp256k1zkp_v0_8_1_context* ctx) {
    /* Client data */
    ...
    /* rustsecp256k1zkp_v0_8_1_musig_session blinded_session; */
    ...
    if (!rustsecp256k1zkp_v0_8_1_musig_pubkey_agg(ctx, NULL, &aggregate_xonly_pubkey, &cache, pubkeys_ptr, 2)) {
        printf("fail\n");
    ...
    if (!rustsecp256k1zkp_v0_8_1_blinded_musig_nonce_process(ctx, &session, &agg_pubnonce, msg, &cache, NULL, blinding_factor)) {
        printf("fail\n");
    ...
    if (!rustsecp256k1zkp_v0_8_1_musig_get_keyaggcoef_and_negation_seckey(ctx, keyaggcoef, &negate_seckey, &cache, &server_pubkey)) {
        printf("fail\n");
        printf("Failed to calculate server key aggregation coefficient\n");
        return 0;
    }

    if (!rustsecp256k1zkp_v0_8_1_musig_partial_sign(ctx, &client_partial_sig, &client_secnonce, &client_keypair, &cache, &session)) {
        printf("fail\n");
    ...
    if (!rustsecp256k1zkp_v0_8_1_musig_partial_sig_verify(ctx, &client_partial_sig, &client_pubnonce, &client_pubkey, &cache, &session)) {
        printf("fail\n");
        printf("Failed to verify client partial signature\n");
        return 0;
    }

    if (!rustsecp256k1zkp_v0_8_1_musig_partial_sig_verify(ctx, &server_partial_sig, &server_pubnonce, &server_pubkey, &cache, &session)) {
        printf("fail\n");
