...
typedef struct {
    rustsecp256k1zkp_v0_8_1_scalar scalar[2];
    rustsecp256k1zkp_v0_8_1_fe fe[4];
    rustsecp256k1zkp_v0_8_1_ge ge[2];
    rustsecp256k1zkp_v0_8_1_gej gej[2];
...
static void bench_setup(void* arg) {
    ...
    rustsecp256k1zkp_v0_8_1_scalar_set_b32(&data->scalar[0], init[0], NULL);
    rustsecp256k1zkp_v0_8_1_scalar_set_b32(&data->scalar[1], init[1], NULL);
    rustsecp256k1zkp_v0_8_1_fe_set_b32_limit(&data->fe[0], init[0]);
    rustsecp256k1zkp_v0_8_1_fe_set_b32_limit(&data->fe[1], init[1]);
    rustsecp256k1zkp_v0_8_1_fe_set_b32_limit(&data->fe[2], init[2]);
    rustsecp256k1zkp_v0_8_1_fe_set_b32_limit(&data->fe[3], init[3]);
    CHECK(rustsecp256k1zkp_v0_8_1_ge_set_xo_var(&data->ge[0], &data->fe[0], 0));
    CHECK(rustsecp256k1zkp_v0_8_1_ge_set_xo_var(&data->ge[1], &data->fe[1], 1));
    rustsecp256k1zkp_v0_8_1_gej_set_ge(&data->gej[0], &data->ge[0]);
    rustsecp256k1zkp_v0_8_1_gej_rescale(&data->gej[0], &data->fe[2]);
    rustsecp256k1zkp_v0_8_1_gej_set_ge(&data->gej[1], &data->ge[1]);
    rustsecp256k1zkp_v0_8_1_gej_rescale(&data->gej[1], &data->fe[3]);
...
static void bench_scalar_add(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        j += rustsecp256k1zkp_v0_8_1_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(j <= iters);
}
...
static void bench_scalar_negate(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_scalar_negate(&data->scalar[0], &data->scalar[0]);
    }
}
...
static void bench_scalar_sqr(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_scalar_sqr(&data->scalar[0], &data->scalar[0]);
    }
}
...
static void bench_scalar_mul(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_scalar_mul(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
}
...
static void bench_scalar_split(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_scalar_split_lambda(&tmp, &data->scalar[1], &data->scalar[0]);
        j += rustsecp256k1zkp_v0_8_1_scalar_add(&data->scalar[0], &tmp, &data->scalar[1]);
    }
    CHECK(j <= iters);
}
...
static void bench_scalar_inverse(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_scalar_inverse(&data->scalar[0], &data->scalar[0]);
        j += rustsecp256k1zkp_v0_8_1_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(j <= iters);
}
...
static void bench_scalar_inverse_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_scalar_inverse_var(&data->scalar[0], &data->scalar[0]);
        j += rustsecp256k1zkp_v0_8_1_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(j <= iters);
}
...
static void bench_field_half(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_fe_half(&data->fe[0]);
    }
}
...
static void bench_field_normalize(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_fe_normalize(&data->fe[0]);
    }
}
...
static void bench_field_normalize_weak(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_fe_normalize_weak(&data->fe[0]);
    }
}
...
static void bench_field_mul(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_fe_mul(&data->fe[0], &data->fe[0], &data->fe[1]);
    }
}
...
static void bench_field_sqr(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_fe_sqr(&data->fe[0], &data->fe[0]);
    }
}
...
static void bench_field_inverse(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_fe_inv(&data->fe[0], &data->fe[0]);
        rustsecp256k1zkp_v0_8_1_fe_add(&data->fe[0], &data->fe[1]);
    }
}
...
static void bench_field_inverse_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_fe_inv_var(&data->fe[0], &data->fe[0]);
        rustsecp256k1zkp_v0_8_1_fe_add(&data->fe[0], &data->fe[1]);
    }
}
...
static void bench_field_sqrt(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        t = data->fe[0];
        j += rustsecp256k1zkp_v0_8_1_fe_sqrt(&data->fe[0], &t);
        rustsecp256k1zkp_v0_8_1_fe_add(&data->fe[0], &data->fe[1]);
    }
    CHECK(j <= iters);
}
...
static void bench_field_is_square_var(void* arg, int iters) {
    int i, j = 0;
    bench_inv *data = (bench_inv*)arg;
    rustsecp256k1zkp_v0_8_1_fe t = data->fe[0];

    for (i = 0; i < iters; i++) {
        j += rustsecp256k1zkp_v0_8_1_fe_is_square_var(&t);
        rustsecp256k1zkp_v0_8_1_fe_add(&t, &data->fe[1]);
...
static void bench_group_double_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_gej_double_var(&data->gej[0], &data->gej[0], NULL);
    }
}
...
static void bench_group_add_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_gej_add_var(&data->gej[0], &data->gej[0], &data->gej[1], NULL);
    }
}
...
static void bench_group_add_affine(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_gej_add_ge(&data->gej[0], &data->gej[0], &data->ge[1]);
    }
}
...
static void bench_group_add_affine_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_gej_add_ge_var(&data->gej[0], &data->gej[0], &data->ge[1], NULL);
    }
}
...
static void bench_group_jacobi_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        j += rustsecp256k1zkp_v0_8_1_gej_has_quad_y_var(&data->gej[0]);
        /* Vary the Y and Z coordinates of the input (the X coordinate doesn't matter to
           rustsecp256k1zkp_v0_8_1_gej_has_quad_y_var). Note that the resulting coordinates will
           ...
           curve). */
        rustsecp256k1zkp_v0_8_1_fe_add(&data->gej[0].y, &data->fe[1]);
        rustsecp256k1zkp_v0_8_1_fe_add(&data->gej[0].z, &data->fe[2]);
        rustsecp256k1zkp_v0_8_1_fe_normalize_var(&data->gej[0].y);
        rustsecp256k1zkp_v0_8_1_fe_normalize_var(&data->gej[0].z);
    }
    CHECK(j <= iters);
}
...
static void bench_group_add_zinv_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_gej_add_zinv_var(&data->gej[0], &data->gej[0], &data->ge[1], &data->gej[0].y);
    }
}
...
static void bench_group_to_affine_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; ++i) {
        rustsecp256k1zkp_v0_8_1_ge_set_gej_var(&data->ge[1], &data->gej[0]);
        ...
        rustsecp256k1zkp_v0_8_1_fe_add(&data->gej[0].x, &data->ge[1].y);
        rustsecp256k1zkp_v0_8_1_fe_add(&data->gej[0].y, &data->fe[2]);
        rustsecp256k1zkp_v0_8_1_fe_add(&data->gej[0].z, &data->ge[1].x);
        rustsecp256k1zkp_v0_8_1_fe_normalize_var(&data->gej[0].x);
        rustsecp256k1zkp_v0_8_1_fe_normalize_var(&data->gej[0].y);
        rustsecp256k1zkp_v0_8_1_fe_normalize_var(&data->gej[0].z);
    }
}
...
static void bench_ecmult_wnaf(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        bits += rustsecp256k1zkp_v0_8_1_ecmult_wnaf(data->wnaf, 256, &data->scalar[0], WINDOW_A);
        overflow += rustsecp256k1zkp_v0_8_1_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(overflow >= 0);
...
static void bench_wnaf_const(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        bits += rustsecp256k1zkp_v0_8_1_wnaf_const(data->wnaf, &data->scalar[0], WINDOW_A, 256);
        overflow += rustsecp256k1zkp_v0_8_1_scalar_add(&data->scalar[0], &data->scalar[0], &data->scalar[1]);
    }
    CHECK(overflow >= 0);
    CHECK(bits <= 256*iters);
...
static void bench_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        rustsecp256k1zkp_v0_8_1_sha256_write(&sha, data->data, 32);
        rustsecp256k1zkp_v0_8_1_sha256_finalize(&sha, data->data);
    }
}
...
static void bench_hmac_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_hmac_sha256_initialize(&hmac, data->data, 32);
        rustsecp256k1zkp_v0_8_1_hmac_sha256_write(&hmac, data->data, 32);
        rustsecp256k1zkp_v0_8_1_hmac_sha256_finalize(&hmac, data->data);
    }
}
...
static void bench_rfc6979_hmac_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_rfc6979_hmac_sha256_initialize(&rng, data->data, 64);
        rustsecp256k1zkp_v0_8_1_rfc6979_hmac_sha256_generate(&rng, data->data, 32);
    }
}
...
static void bench_context(void* arg, int iters) {
    ...
    (void)arg;
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_context_destroy(rustsecp256k1zkp_v0_8_1_context_create(SECP256K1_CONTEXT_NONE));
    }
}
