...
typedef struct {
    ...
    const void* data;
} rustsecp256k1zkp_v0_8_1_callback;

static SECP256K1_INLINE void rustsecp256k1zkp_v0_8_1_callback_call(const rustsecp256k1zkp_v0_8_1_callback * const cb, const char * const text) {
    cb->fn(text, (void*)cb->data);
}

#ifndef USE_EXTERNAL_DEFAULT_CALLBACKS
static void rustsecp256k1zkp_v0_8_1_default_illegal_callback_fn(const char* str, void* data) {
    (void)data;
    fprintf(stderr, "[libsecp256k1] illegal argument: %s\n", str);
    abort();
}
static void rustsecp256k1zkp_v0_8_1_default_error_callback_fn(const char* str, void* data) {
    (void)data;
    fprintf(stderr, "[libsecp256k1] internal consistency check failed: %s\n", str);
    abort();
}
#else
void rustsecp256k1zkp_v0_8_1_default_illegal_callback_fn(const char* str, void* data);
void rustsecp256k1zkp_v0_8_1_default_error_callback_fn(const char* str, void* data);
#endif

static const rustsecp256k1zkp_v0_8_1_callback default_illegal_callback = {
    rustsecp256k1zkp_v0_8_1_default_illegal_callback_fn,
    NULL
};

static const rustsecp256k1zkp_v0_8_1_callback default_error_callback = {
    rustsecp256k1zkp_v0_8_1_default_error_callback_fn,
    NULL
};
...
SECP256K1_INLINE static int rustsecp256k1zkp_v0_8_1_sign_and_abs64(uint64_t *out, int64_t in) {
...
SECP256K1_INLINE static int rustsecp256k1zkp_v0_8_1_clz64_var(uint64_t x) {
    int ret;
    if (!x) {
        return 64;
    }
...
static SECP256K1_INLINE void rustsecp256k1zkp_v0_8_1_memczero(void *s, size_t len, int flag) {
 ...
 */
static SECP256K1_INLINE int rustsecp256k1zkp_v0_8_1_memcmp_var(const void *s1, const void *s2, size_t n) {
...
static SECP256K1_INLINE void rustsecp256k1zkp_v0_8_1_int_cmov(int *r, const int *a, int flag) {
 ...
 * rustsecp256k1zkp_v0_8_1_ctz32_var, but permits it to be tested separately. */
static SECP256K1_INLINE int rustsecp256k1zkp_v0_8_1_ctz32_var_debruijn(uint32_t x) {
 ...
 * This function is only intended to be used as fallback for
 * rustsecp256k1zkp_v0_8_1_ctz64_var, but permits it to be tested separately. */
static SECP256K1_INLINE int rustsecp256k1zkp_v0_8_1_ctz64_var_debruijn(uint64_t x) {
...
/* Determine the number of trailing zero bits in a (non-zero) 32-bit x. */
static SECP256K1_INLINE int rustsecp256k1zkp_v0_8_1_ctz32_var(uint32_t x) {
    VERIFY_CHECK(x != 0);
    ...
    return rustsecp256k1zkp_v0_8_1_ctz32_var_debruijn(x);
#endif
}

/* Determine the number of trailing zero bits in a (non-zero) 64-bit x. */
static SECP256K1_INLINE int rustsecp256k1zkp_v0_8_1_ctz64_var(uint64_t x) {
    VERIFY_CHECK(x != 0);
#if (__has_builtin(__builtin_ctzl) || SECP256K1_GNUC_PREREQ(3,4))
...
#else
    /* If no suitable CTZ builtin is available, use a (variable time) software emulation. */
    return rustsecp256k1zkp_v0_8_1_ctz64_var_debruijn(x);
#endif
}

/* Read a uint32_t in big endian */
SECP256K1_INLINE static uint32_t rustsecp256k1zkp_v0_8_1_read_be32(const unsigned char* p) {
...
SECP256K1_INLINE static void rustsecp256k1zkp_v0_8_1_write_be32(unsigned char* p, uint32_t x) {
    p[3] = x;
    p[2] = x >>  8;
    p[1] = x >> 16;
    p[0] = x >> 24;
...
SECP256K1_INLINE static uint64_t rustsecp256k1zkp_v0_8_1_read_be64(const unsigned char* p) {
...
SECP256K1_INLINE static void rustsecp256k1zkp_v0_8_1_write_be64(unsigned char* p, uint64_t x) {
    p[7] = x;
    p[6] = x >>  8;
    p[5] = x >> 16;
    p[4] = x >> 24;
