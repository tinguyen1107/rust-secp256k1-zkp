...
static void bench_recover(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(rustsecp256k1zkp_v0_8_1_ecdsa_recoverable_signature_parse_compact(data->ctx, &sig, data->sig, i % 2));
        CHECK(rustsecp256k1zkp_v0_8_1_ecdsa_recover(data->ctx, &pubkey, &sig, data->msg));
        CHECK(rustsecp256k1zkp_v0_8_1_ec_pubkey_serialize(data->ctx, pubkeyc, &pubkeylen, &pubkey, SECP256K1_EC_COMPRESSED));
        for (j = 0; j < 32; j++) {
