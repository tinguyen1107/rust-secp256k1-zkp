/**********************************************************************
 * Copyright (c) 2015 Gregory Maxwell                                 *
 * Distributed under the MIT software license, see the accompanying   *
 * file COPYING or http://www.opensource.org/licenses/mit-license.php.*
 **********************************************************************/

#ifndef SECP256K1_MODULE_RANGEPROOF_TESTS_H
#define SECP256K1_MODULE_RANGEPROOF_TESTS_H

#include <string.h>

#include "../../group.h"
#include "../../scalar.h"
#include "../../testrand.h"
#include "../../util.h"

#include "../../../include/secp256k1_rangeproof.h"

static void test_rangeproof_api(const int32_t *ecount) {
    unsigned char proof[5134];
    unsigned char blind[32];
    rustsecp256k1zkp_v0_8_1_pedersen_commitment commit;
    uint64_t vmin = rustsecp256k1zkp_v0_8_1_testrand32();
    uint64_t val = vmin + rustsecp256k1zkp_v0_8_1_testrand32();
    size_t len = sizeof(proof);
    /* we'll switch to dylan thomas for this one */
    const unsigned char message[68] = "My tears are like the quiet drift / Of petals from some magic rose;";
    size_t mlen = sizeof(message);
    const unsigned char ext_commit[72] = "And all my grief flows from the rift / Of unremembered skies and snows.";
    size_t ext_commit_len = sizeof(ext_commit);

    rustsecp256k1zkp_v0_8_1_testrand256(blind);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, val, rustsecp256k1zkp_v0_8_1_generator_h));

    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, val, message, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 1);
    CHECK(*ecount == 0);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(STATIC_CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, val, message, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(*ecount == 1);

    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, NULL, &len, vmin, &commit, blind, commit.data, 0, 0, val, message, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(*ecount == 2);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, NULL, vmin, &commit, blind, commit.data, 0, 0, val, message, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(*ecount == 3);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, NULL, blind, commit.data, 0, 0, val, message, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(*ecount == 4);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, NULL, commit.data, 0, 0, val, message, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(*ecount == 5);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, NULL, 0, 0, val, message, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(*ecount == 6);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, vmin - 1, message, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(*ecount == 6);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, val, NULL, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(*ecount == 7);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, val, NULL, 0, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) != 0);
    CHECK(*ecount == 7);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, val, NULL, 0, NULL, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(*ecount == 8);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, val, NULL, 0, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h) != 0);
    CHECK(*ecount == 8);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, val, NULL, 0, NULL, 0, NULL) == 0);
    CHECK(*ecount == 9);

    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, val, message, mlen, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) != 0);
    {
        int exp;
        int mantissa;
        uint64_t min_value;
        uint64_t max_value;
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_info(CTX, &exp, &mantissa, &min_value, &max_value, proof, len) != 0);
        CHECK(exp == 0);
        CHECK(((uint64_t) 1 << mantissa) > val - vmin);
        CHECK(((uint64_t) 1 << (mantissa - 1)) <= val - vmin);
        CHECK(min_value == vmin);
        CHECK(max_value >= val);

        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_info(CTX, NULL, &mantissa, &min_value, &max_value, proof, len) == 0);
        CHECK(*ecount == 10);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_info(CTX, &exp, NULL, &min_value, &max_value, proof, len) == 0);
        CHECK(*ecount == 11);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_info(CTX, &exp, &mantissa, NULL, &max_value, proof, len) == 0);
        CHECK(*ecount == 12);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_info(CTX, &exp, &mantissa, &min_value, NULL, proof, len) == 0);
        CHECK(*ecount == 13);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_info(CTX, &exp, &mantissa, &min_value, &max_value, NULL, len) == 0);
        CHECK(*ecount == 14);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_info(CTX, &exp, &mantissa, &min_value, &max_value, proof, 0) == 0);
        CHECK(*ecount == 14);
    }
    {
        uint64_t min_value;
        uint64_t max_value;
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &min_value, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 1);
        CHECK(*ecount == 14);

        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, NULL, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 15);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &min_value, NULL, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 16);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &min_value, &max_value, NULL, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 17);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &min_value, &max_value, &commit, NULL, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 18);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &min_value, &max_value, &commit, proof, 0, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 18);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &min_value, &max_value, &commit, proof, len, NULL, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 19);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &min_value, &max_value, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 19);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &min_value, &max_value, &commit, proof, len, NULL, 0, NULL) == 0);
        CHECK(*ecount == 20);
    }
    {
        unsigned char blind_out[32];
        unsigned char message_out[68];
        uint64_t value_out;
        uint64_t min_value;
        uint64_t max_value;
        size_t message_len = sizeof(message_out);

        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, message_out, &message_len, commit.data, &min_value, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 1);
        CHECK(*ecount == 20);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(STATIC_CTX, blind_out, &value_out, message_out, &message_len, commit.data, &min_value, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 21);

        CHECK(min_value == vmin);
        CHECK(max_value >= val);
        CHECK(value_out == val);
        CHECK(message_len == sizeof(message_out));
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(message, message_out, sizeof(message_out)) == 0);

        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, NULL, &value_out, message_out, &message_len, commit.data, &min_value, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) != 0);
        CHECK(*ecount == 21);  /* blindout may be NULL */
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, NULL, message_out, &message_len, commit.data, &min_value, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) != 0);
        CHECK(*ecount == 21);  /* valueout may be NULL */
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, &message_len, commit.data, &min_value, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 22);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, commit.data, &min_value, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) != 0);
        CHECK(*ecount == 22);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, NULL, &min_value, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 23);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, commit.data, NULL, &max_value, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 24);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, commit.data, &min_value, NULL, &commit, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 25);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, commit.data, &min_value, &max_value, NULL, proof, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 26);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, commit.data, &min_value, &max_value, &commit, NULL, len, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 27);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, commit.data, &min_value, &max_value, &commit, proof, 0, ext_commit, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 27);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, commit.data, &min_value, &max_value, &commit, proof, len, NULL, ext_commit_len, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 28);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, commit.data, &min_value, &max_value, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
        CHECK(*ecount == 28);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, NULL, 0, commit.data, &min_value, &max_value, &commit, proof, len, NULL, 0, NULL) == 0);
        CHECK(*ecount == 29);
    }

    /* This constant is hardcoded in these tests and elsewhere, so we
     * consider it to be part of the API and test it here. */
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, 0, 64) == 5134);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, UINT64_MAX, 0) == 5134);
}

static void test_api(void) {
    int32_t ecount;
    int i;

    rustsecp256k1zkp_v0_8_1_context_set_error_callback(CTX, counting_illegal_callback_fn, &ecount);
    rustsecp256k1zkp_v0_8_1_context_set_error_callback(STATIC_CTX, counting_illegal_callback_fn, &ecount);
    rustsecp256k1zkp_v0_8_1_context_set_illegal_callback(CTX, counting_illegal_callback_fn, &ecount);
    rustsecp256k1zkp_v0_8_1_context_set_illegal_callback(STATIC_CTX, counting_illegal_callback_fn, &ecount);

    for (i = 0; i < COUNT; i++) {
        ecount = 0;
        test_rangeproof_api(&ecount);
    }

    rustsecp256k1zkp_v0_8_1_context_set_error_callback(CTX, NULL, NULL);
    rustsecp256k1zkp_v0_8_1_context_set_error_callback(STATIC_CTX, NULL, NULL);
    rustsecp256k1zkp_v0_8_1_context_set_illegal_callback(CTX, NULL, NULL);
    rustsecp256k1zkp_v0_8_1_context_set_illegal_callback(STATIC_CTX, NULL, NULL);
}

static void test_borromean(void) {
    unsigned char e0[32];
    rustsecp256k1zkp_v0_8_1_scalar s[64];
    rustsecp256k1zkp_v0_8_1_gej pubs[64];
    rustsecp256k1zkp_v0_8_1_scalar k[8];
    rustsecp256k1zkp_v0_8_1_scalar sec[8];
    rustsecp256k1zkp_v0_8_1_ge ge;
    rustsecp256k1zkp_v0_8_1_scalar one;
    unsigned char m[32];
    size_t rsizes[8];
    size_t secidx[8];
    size_t nrings;
    size_t i;
    size_t j;
    int c;
    rustsecp256k1zkp_v0_8_1_testrand256_test(m);
    nrings = 1 + (rustsecp256k1zkp_v0_8_1_testrand32()&7);
    c = 0;
    rustsecp256k1zkp_v0_8_1_scalar_set_int(&one, 1);
    if (rustsecp256k1zkp_v0_8_1_testrand32()&1) {
        rustsecp256k1zkp_v0_8_1_scalar_negate(&one, &one);
    }
    for (i = 0; i < nrings; i++) {
        rsizes[i] = 1 + (rustsecp256k1zkp_v0_8_1_testrand32()&7);
        secidx[i] = rustsecp256k1zkp_v0_8_1_testrand32() % rsizes[i];
        random_scalar_order(&sec[i]);
        random_scalar_order(&k[i]);
        if(rustsecp256k1zkp_v0_8_1_testrand32()&7) {
            sec[i] = one;
        }
        if(rustsecp256k1zkp_v0_8_1_testrand32()&7) {
            k[i] = one;
        }
        for (j = 0; j < rsizes[i]; j++) {
            random_scalar_order(&s[c + j]);
            if(rustsecp256k1zkp_v0_8_1_testrand32()&7) {
                s[i] = one;
            }
            if (j == secidx[i]) {
                rustsecp256k1zkp_v0_8_1_ecmult_gen(&CTX->ecmult_gen_ctx, &pubs[c + j], &sec[i]);
            } else {
                random_group_element_test(&ge);
                random_group_element_jacobian_test(&pubs[c + j],&ge);
            }
        }
        c += rsizes[i];
    }
    CHECK(rustsecp256k1zkp_v0_8_1_borromean_sign(&CTX->ecmult_gen_ctx, e0, s, pubs, k, sec, rsizes, secidx, nrings, m, 32));
    CHECK(rustsecp256k1zkp_v0_8_1_borromean_verify(NULL, e0, s, pubs, rsizes, nrings, m, 32));
    i = rustsecp256k1zkp_v0_8_1_testrand32() % c;
    rustsecp256k1zkp_v0_8_1_scalar_negate(&s[i],&s[i]);
    CHECK(!rustsecp256k1zkp_v0_8_1_borromean_verify(NULL, e0, s, pubs, rsizes, nrings, m, 32));
    rustsecp256k1zkp_v0_8_1_scalar_negate(&s[i],&s[i]);
    rustsecp256k1zkp_v0_8_1_scalar_set_int(&one, 1);
    for(j = 0; j < 4; j++) {
        i = rustsecp256k1zkp_v0_8_1_testrand32() % c;
        if (rustsecp256k1zkp_v0_8_1_testrand32() & 1) {
            rustsecp256k1zkp_v0_8_1_gej_double_var(&pubs[i],&pubs[i], NULL);
        } else {
            rustsecp256k1zkp_v0_8_1_scalar_add(&s[i],&s[i],&one);
        }
        CHECK(!rustsecp256k1zkp_v0_8_1_borromean_verify(NULL, e0, s, pubs, rsizes, nrings, m, 32));
    }
}

static void test_rangeproof(void) {
    const uint64_t testvs[11] = {0, 1, 5, 11, 65535, 65537, INT32_MAX, UINT32_MAX, INT64_MAX - 1, INT64_MAX, UINT64_MAX};
    rustsecp256k1zkp_v0_8_1_pedersen_commitment commit;
    rustsecp256k1zkp_v0_8_1_pedersen_commitment commit2;
    unsigned char proof[5134 + 1]; /* One additional byte to test if trailing bytes are rejected */
    unsigned char blind[32];
    unsigned char blindout[32];
    unsigned char message[4096];
    size_t mlen;
    uint64_t v;
    uint64_t vout;
    uint64_t vmin;
    uint64_t minv;
    uint64_t maxv;
    size_t len;
    size_t i;
    size_t j;
    size_t k;
    /* Short message is a Simone de Beauvoir quote */
    const unsigned char message_short[120] = "When I see my own likeness in the depths of someone else's consciousness,  I always experience a moment of panic.";
    /* Long message is 0xA5 with a bunch of this quote in the middle */
    unsigned char message_long[3968];
    memset(message_long, 0xa5, sizeof(message_long));
    for (i = 1200; i < 3600; i += 120) {
        memcpy(&message_long[i], message_short, sizeof(message_short));
    }

    rustsecp256k1zkp_v0_8_1_testrand256(blind);
    for (i = 0; i < 11; i++) {
        v = testvs[i];
        CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, v, rustsecp256k1zkp_v0_8_1_generator_h));
        for (vmin = 0; vmin < (i<9 && i > 0 ? 2 : 1); vmin++) {
            const unsigned char *input_message = NULL;
            size_t input_message_len = 0;
            /* vmin is always either 0 or 1; if it is 1, then we have no room for a message.
             * If it's 0, we use "minimum encoding" and only have room for a small message when
             * `testvs[i]` is >= 4; for a large message when it's >= 2^32. */
            if (vmin == 0 && i > 2) {
                input_message = message_short;
                input_message_len = sizeof(message_short);
            }
            if (vmin == 0 && i > 7) {
                input_message = message_long;
                input_message_len = sizeof(message_long);
            }
            len = 5134;
            CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, 0, 0, v, input_message, input_message_len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
            CHECK(len <= 5134);
            CHECK(len <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, v, 0));
            mlen = 4096;
            CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blindout, &vout, message, &mlen, commit.data, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
            if (input_message != NULL) {
                CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(message, input_message, input_message_len) == 0);
            }
            for (j = input_message_len; j < mlen; j++) {
                CHECK(message[j] == 0);
            }
            CHECK(mlen <= 4096);
            CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blindout, blind, 32) == 0);
            CHECK(vout == v);
            CHECK(minv <= v);
            CHECK(maxv >= v);
            len = 5134;
            CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, v, &commit, blind, commit.data, -1, 64, v, NULL, 0, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
            CHECK(len <= 73);
            CHECK(len <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, v, 0));
            CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blindout, &vout, NULL, NULL, commit.data, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
            CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blindout, blind, 32) == 0);
            CHECK(vout == v);
            CHECK(minv == v);
            CHECK(maxv == v);

            /* Check with a committed message */
            len = 5134;
            CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, v, &commit, blind, commit.data, -1, 64, v, NULL, 0, message_short, sizeof(message_short), rustsecp256k1zkp_v0_8_1_generator_h));
            CHECK(len <= 73);
            CHECK(len <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, v, 0));
            CHECK(!rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blindout, &vout, NULL, NULL, commit.data, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
            CHECK(!rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blindout, &vout, NULL, NULL, commit.data, &minv, &maxv, &commit, proof, len, message_long, sizeof(message_long), rustsecp256k1zkp_v0_8_1_generator_h));
            CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blindout, &vout, NULL, NULL, commit.data, &minv, &maxv, &commit, proof, len, message_short, sizeof(message_short), rustsecp256k1zkp_v0_8_1_generator_h));
            CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blindout, blind, 32) == 0);
            CHECK(vout == v);
            CHECK(minv == v);
            CHECK(maxv == v);
        }
    }
    rustsecp256k1zkp_v0_8_1_testrand256(blind);
    v = INT64_MAX - 1;
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, v, rustsecp256k1zkp_v0_8_1_generator_h));
    for (i = 0; i < 19; i++) {
        len = 5134;
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, 0, &commit, blind, commit.data, i, 0, v, NULL, 0, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(len <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, v, 0));
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(len <= 5134);
        CHECK(minv <= v);
        CHECK(maxv >= v);
        /* Make sure it fails when validating with a committed message */
        CHECK(!rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &minv, &maxv, &commit, proof, len, message_short, sizeof(message_short), rustsecp256k1zkp_v0_8_1_generator_h));
    }
    rustsecp256k1zkp_v0_8_1_testrand256(blind);
    {
        /*Malleability test.*/
        v = rustsecp256k1zkp_v0_8_1_testrandi64(0, 255);
        CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, v, rustsecp256k1zkp_v0_8_1_generator_h));
        len = 5134;
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, 0, &commit, blind, commit.data, 0, 3, v, NULL, 0, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(len <= 5134);
        CHECK(len <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, v, 3));
        /* Test if trailing bytes are rejected. */
        proof[len] = v;
        CHECK(!rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &minv, &maxv, &commit, proof, len + 1, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        for (i = 0; i < len*8; i++) {
            proof[i >> 3] ^= 1 << (i & 7);
            CHECK(!rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
            proof[i >> 3] ^= 1 << (i & 7);
        }
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(minv <= v);
        CHECK(maxv >= v);
    }
    memcpy(&commit2, &commit, sizeof(commit));
    for (i = 0; i < (size_t) COUNT; i++) {
        int exp;
        int min_bits;
        v = rustsecp256k1zkp_v0_8_1_testrandi64(0, UINT64_MAX >> (rustsecp256k1zkp_v0_8_1_testrand32()&63));
        vmin = 0;
        if ((v < INT64_MAX) && (rustsecp256k1zkp_v0_8_1_testrand32()&1)) {
            vmin = rustsecp256k1zkp_v0_8_1_testrandi64(0, v);
        }
        rustsecp256k1zkp_v0_8_1_testrand256(blind);
        CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, v, rustsecp256k1zkp_v0_8_1_generator_h));
        len = 5134;
        exp = (int)rustsecp256k1zkp_v0_8_1_testrandi64(0,18)-(int)rustsecp256k1zkp_v0_8_1_testrandi64(0,18);
        if (exp < 0) {
            exp = -exp;
        }
        min_bits = (int)rustsecp256k1zkp_v0_8_1_testrandi64(0,64)-(int)rustsecp256k1zkp_v0_8_1_testrandi64(0,64);
        if (min_bits < 0) {
            min_bits = -min_bits;
        }
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, vmin, &commit, blind, commit.data, exp, min_bits, v, NULL, 0, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(len <= 5134);
        CHECK(len <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, v, min_bits));
        mlen = 4096;
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blindout, &vout, message, &mlen, commit.data, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        for (j = 0; j < mlen; j++) {
            CHECK(message[j] == 0);
        }
        CHECK(mlen <= 4096);
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blindout, blind, 32) == 0);

        CHECK(minv <= v);
        CHECK(maxv >= v);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blindout, &vout, NULL, NULL, commit.data, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        memcpy(&commit2, &commit, sizeof(commit));
    }
    for (j = 0; j < 3; j++) {
        for (i = 0; i < 96; i++) {
            rustsecp256k1zkp_v0_8_1_testrand256(&proof[i * 32]);
        }
        for (k = 0; k < 128; k += 3) {
            len = k;
            CHECK(!rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &minv, &maxv, &commit2, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        }
        len = rustsecp256k1zkp_v0_8_1_testrandi64(0, 3072);
        CHECK(!rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &minv, &maxv, &commit2, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
    }
}

static void test_rangeproof_null_blinder(void) {
    unsigned char proof[5134];
    const unsigned char blind[32] = { 0 };
    const uint64_t v = 1111;
    uint64_t minv, maxv;
    rustsecp256k1zkp_v0_8_1_pedersen_commitment commit;
    size_t len;

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, v, rustsecp256k1zkp_v0_8_1_generator_h));

    /* Try a 32-bit proof; should work */
    len = 5134;
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, 1, &commit, blind, commit.data, 0, 32, v, NULL, 0, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
    CHECK(minv == 1);
    CHECK(maxv == 1ULL << 32);

    /* Try a 3-bit proof; should work */
    len = 5134;
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, v - 1, &commit, blind, commit.data, 0, 3, v, NULL, 0, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(CTX, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
    CHECK(minv == 1110);
    CHECK(maxv == 1117);

    /* But a 2-bits will not because then it does not have any subcommitments (which rerandomize
     * the blinding factors that get passed into the borromean logic ... passing 0s will fail) */
    len = 5134;
    CHECK(!rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, v - 1, &commit, blind, commit.data, 0, 2, v, NULL, 0, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));

    /* Rewinding with 3-bits works */
    {
        uint64_t value_out;
        unsigned char msg[128];
        unsigned char msg_out[128];
        unsigned char blind_out[32];
        size_t msg_len = sizeof(msg);

        len = 1000;
        rustsecp256k1zkp_v0_8_1_testrand256(msg);
        rustsecp256k1zkp_v0_8_1_testrand256(&msg[32]);
        rustsecp256k1zkp_v0_8_1_testrand256(&msg[64]);
        rustsecp256k1zkp_v0_8_1_testrand256(&msg[96]);
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &len, v, &commit, blind, commit.data, 0, 3, v, msg, sizeof(msg), NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(CTX, blind_out, &value_out, msg_out, &msg_len, commit.data, &minv, &maxv, &commit, proof, len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h) != 0);
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blind, blind_out, sizeof(blind)) == 0);
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(msg, msg_out, sizeof(msg)) == 0);
        CHECK(value_out == v);
        CHECK(minv == v);
        CHECK(maxv == v + 7);
    }
}

static void test_single_value_proof(uint64_t val) {
    unsigned char proof[5000];
    rustsecp256k1zkp_v0_8_1_pedersen_commitment commit;
    unsigned char blind[32];
    unsigned char blind_out[32];
    unsigned char nonce[32];
    const unsigned char message[1] = " "; /* no message will fit into a single-value proof */
    unsigned char message_out[sizeof(proof)] = { 0 };
    size_t plen = sizeof(proof);
    uint64_t min_val_out = 0;
    uint64_t max_val_out = 0;

    uint64_t val_out = 0;
    size_t m_len_out = 0;

    rustsecp256k1zkp_v0_8_1_testrand256(blind);
    rustsecp256k1zkp_v0_8_1_testrand256(nonce);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, val, rustsecp256k1zkp_v0_8_1_generator_h));

    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(
        CTX,
        proof, &plen,
        val, /* min_val */
        &commit, blind, nonce,
        -1, /* exp: -1 is magic value to indicate a single-value proof */
        0, /* min_bits */
        val, /* val */
        message, sizeof(message),  /* Will cause this to fail */
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ) == 0);

    plen = sizeof(proof);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(
        CTX,
        proof, &plen,
        val, /* min_val */
        &commit, blind, nonce,
        -1, /* exp: -1 is magic value to indicate a single-value proof */
        0, /* min_bits */
        val, /* val */
        NULL, 0,
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ) == 1);
    CHECK(plen <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, val, 0));

    /* Different proof sizes are unfortunate but is caused by `min_value` of
     * zero being special-cased and encoded more efficiently. */
    if (val == 0) {
        CHECK(plen == 65);
    } else {
        CHECK(plen == 73);
    }

    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(
        CTX,
        &min_val_out, &max_val_out,
        &commit,
        proof, plen,
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ) == 1);
    CHECK(min_val_out == val);
    CHECK(max_val_out == val);

    memset(message_out, 0, sizeof(message_out));
    m_len_out = sizeof(message_out);
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(
        CTX,
        blind_out, &val_out,
        message_out, &m_len_out,
        nonce,
        &min_val_out, &max_val_out,
        &commit,
        proof, plen,
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ));
    CHECK(val_out == val);
    CHECK(min_val_out == val);
    CHECK(max_val_out == val);
    CHECK(m_len_out == 0);
    CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blind, blind_out, 32) == 0);
    for (m_len_out = 0; m_len_out < sizeof(message_out); m_len_out++) {
        CHECK(message_out[m_len_out] == 0);
    }
}

#define MAX_N_GENS	30
static void test_multiple_generators(void) {
    const size_t n_inputs = (rustsecp256k1zkp_v0_8_1_testrand32() % (MAX_N_GENS / 2)) + 1;
    const size_t n_outputs = (rustsecp256k1zkp_v0_8_1_testrand32() % (MAX_N_GENS / 2)) + 1;
    const size_t n_generators = n_inputs + n_outputs;
    unsigned char *generator_blind[MAX_N_GENS];
    unsigned char *pedersen_blind[MAX_N_GENS];
    rustsecp256k1zkp_v0_8_1_generator generator[MAX_N_GENS];
    rustsecp256k1zkp_v0_8_1_pedersen_commitment commit[MAX_N_GENS];
    const rustsecp256k1zkp_v0_8_1_pedersen_commitment *commit_ptr[MAX_N_GENS];
    size_t i;
    int64_t total_value;
    uint64_t value[MAX_N_GENS];

    rustsecp256k1zkp_v0_8_1_scalar s;

    unsigned char generator_seed[32];
    random_scalar_order(&s);
    rustsecp256k1zkp_v0_8_1_scalar_get_b32(generator_seed, &s);
    /* Create all the needed generators */
    for (i = 0; i < n_generators; i++) {
        generator_blind[i] = (unsigned char*) malloc(32);
        pedersen_blind[i] = (unsigned char*) malloc(32);

        random_scalar_order(&s);
        rustsecp256k1zkp_v0_8_1_scalar_get_b32(generator_blind[i], &s);
        random_scalar_order(&s);
        rustsecp256k1zkp_v0_8_1_scalar_get_b32(pedersen_blind[i], &s);

        CHECK(rustsecp256k1zkp_v0_8_1_generator_generate_blinded(CTX, &generator[i], generator_seed, generator_blind[i]));

        commit_ptr[i] = &commit[i];
    }

    /* Compute all the values -- can be positive or negative */
    total_value = 0;
    for (i = 0; i < n_outputs; i++) {
        value[n_inputs + i] = rustsecp256k1zkp_v0_8_1_testrandi64(0, INT64_MAX - total_value);
        total_value += value[n_inputs + i];
    }
    for (i = 0; i < n_inputs - 1; i++) {
        value[i] = rustsecp256k1zkp_v0_8_1_testrandi64(0, total_value);
        total_value -= value[i];
    }
    value[i] = total_value;

    /* Correct for blinding factors and do the commitments */
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_generator_blind_sum(CTX, value, (const unsigned char * const *) generator_blind, pedersen_blind, n_generators, n_inputs));
    for (i = 0; i < n_generators; i++) {
        CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit[i], pedersen_blind[i], value[i], &generator[i]));
    }

    /* Verify */
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, &commit_ptr[0], n_inputs, &commit_ptr[n_inputs], n_outputs));

    /* Cleanup */
    for (i = 0; i < n_generators; i++) {
        free(generator_blind[i]);
        free(pedersen_blind[i]);
    }
}

static void test_rangeproof_fixed_vectors(void) {
    size_t i;
    unsigned char blind[32];
    uint64_t value;
    uint64_t min_value;
    uint64_t max_value;
    rustsecp256k1zkp_v0_8_1_pedersen_commitment pc;
    unsigned char message[4000] = {0};
    size_t m_len = sizeof(message);

    /* Vector 1: no message */
{
    static const unsigned char vector_1[] = {
        
    };
    static const unsigned char commit_1[] = {
        0x08,
        0xf5, 0x1e, 0x0d, 0xc5, 0x86, 0x78, 0x51, 0xa9, 0x00, 0x00, 0xef, 0x4d, 0xe2, 0x94, 0x60, 0x89,
        0x83, 0x04, 0xb4, 0x0e, 0x90, 0x10, 0x05, 0x1c, 0x7f, 0xd7, 0x33, 0x92, 0x1f, 0xe7, 0x74, 0x59
    };
    static const unsigned char blind_1[] = {
        0x98, 0x44, 0xfc, 0x7a, 0x64, 0xa9, 0xca, 0xdf, 0xf3, 0x2f, 0x9f, 0x02, 0xba, 0x46, 0xc7, 0xd9,
        0x77, 0x47, 0xa4, 0xd3, 0x53, 0x17, 0xc6, 0x44, 0x30, 0x73, 0x84, 0xeb, 0x1f, 0xbe, 0xa1, 0xfb
    };

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commitment_parse(CTX, &pc, commit_1));
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(
        CTX,
        &min_value, &max_value,
        &pc,
        vector_1, sizeof(vector_1),
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ));
    CHECK(min_value == 86);
    CHECK(max_value == 25586);

    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(
        CTX,
        blind, &value,
        message, &m_len,
        pc.data,
        &min_value, &max_value,
        &pc,
        vector_1, sizeof(vector_1),
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ));

    CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blind, blind_1, 32) == 0);
    CHECK(value == 86);
    CHECK(min_value == 86);
    CHECK(max_value == 25586);
    CHECK(m_len == 448); /* length of the sidechannel in the proof */
    for (i = 0; i < m_len; i++) {
        /* No message encoded in this vector */
        CHECK(message[i] == 0);
    }
}

    /* Vector 2: embedded message */
{
    static const unsigned char vector_2[] = {
        
    };
    static const unsigned char commit_2[] = {
        0x09,
        0x25, 0xa4, 0xbd, 0xc4, 0x57, 0x69, 0xeb, 0x4f, 0x34, 0x0f, 0xea, 0xb8, 0xe4, 0x72, 0x04, 0x54,
        0x06, 0xe5, 0xd6, 0x85, 0x15, 0x42, 0xea, 0x6e, 0x1d, 0x11, 0x11, 0x9c, 0x56, 0xf8, 0x10, 0x45
    };
    static const unsigned char blind_2[] = {
        0xdc, 0x79, 0x07, 0x89, 0x2d, 0xc4, 0xe3, 0x76, 0xf9, 0x13, 0x38, 0xd6, 0x4b, 0x46, 0xed, 0x9d,
        0x9b, 0xf6, 0x70, 0x3d, 0x04, 0xcf, 0x96, 0x8c, 0xfd, 0xb5, 0xff, 0x0a, 0x06, 0xc7, 0x08, 0x8b
    };
    static const unsigned char message_2[] = "When I see my own likeness in the depths of someone else's consciousness,  I always experience a moment of panic.";

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commitment_parse(CTX, &pc, commit_2));
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(
        CTX,
        &min_value, &max_value,
        &pc,
        vector_2, sizeof(vector_2),
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ));
    CHECK(min_value == 0);
    CHECK(max_value == 15);

    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(
        CTX,
        blind, &value,
        message, &m_len,
        pc.data,
        &min_value, &max_value,
        &pc,
        vector_2, sizeof(vector_2),
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ));

    CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blind, blind_2, 32) == 0);
    CHECK(value == 11);
    CHECK(min_value == 0);
    CHECK(max_value == 15);
    CHECK(m_len == 192); /* length of the sidechannel in the proof */
    CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(message, message_2, sizeof(message_2)) == 0);
    for (i = sizeof(message_2); i < m_len; i++) {
        CHECK(message[i] == 0);
    }
}

    /* Vector 3: single-value proof of UINT64_MAX */
{
    static const unsigned char vector_3[] = {
        0x20, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xdc, 0x7d, 0x0b, 0x79, 0x0e, 0xaf, 0x41,
        0xa5, 0x8e, 0x9b, 0x0c, 0x5b, 0xa3, 0xee, 0x7d, 0xfd, 0x3d, 0x6b, 0xf3, 0xac, 0x04, 0x8a, 0x43,
        0x75, 0xb0, 0xb7, 0x0e, 0x92, 0xd7, 0xdf, 0xf0, 0x76, 0xc4, 0xa5, 0xb6, 0x2f, 0xf1, 0xb5, 0xfb,
        0xb4, 0xb6, 0x29, 0xea, 0x34, 0x9b, 0x16, 0x30, 0x0d, 0x06, 0xf1, 0xb4, 0x3f, 0x0d, 0x73, 0x59,
        0x75, 0xbf, 0x5d, 0x19, 0x59, 0xef, 0x11, 0xf0, 0xbf
    };
    static const unsigned char commit_3[] = {
        0x08,
        0xc7, 0xea, 0x40, 0x7d, 0x26, 0x38, 0xa2, 0x99, 0xb9, 0x40, 0x22, 0x78, 0x17, 0x57, 0x65, 0xb3,
        0x36, 0x82, 0x18, 0x42, 0xc5, 0x57, 0x04, 0x5e, 0x58, 0x5e, 0xf6, 0x40, 0x8b, 0x24, 0x73, 0x10
    };
    static const unsigned char nonce_3[] = {
        0x84, 0x50, 0x94, 0x69, 0xa3, 0x4b, 0x6c, 0x62, 0x1a, 0xc7, 0xe2, 0x0e, 0x07, 0x9a, 0x6f, 0x85,
        0x5f, 0x26, 0x50, 0xcd, 0x88, 0x5a, 0x9f, 0xaa, 0x23, 0x5e, 0x0a, 0xe0, 0x7e, 0xc5, 0xe9, 0xf1
    };
    static const unsigned char blind_3[] = {
        0x68, 0x89, 0x47, 0x8c, 0x77, 0xec, 0xcc, 0x2b, 0x65, 0x01, 0x78, 0x6b, 0x06, 0x8b, 0x38, 0x94,
        0xc0, 0x6b, 0x9b, 0x4c, 0x02, 0xa6, 0xc8, 0xf6, 0xc0, 0x34, 0xea, 0x35, 0x57, 0xf4, 0xe1, 0x37
    };

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commitment_parse(CTX, &pc, commit_3));
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(
        CTX,
        &min_value, &max_value,
        &pc,
        vector_3, sizeof(vector_3),
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ));
    CHECK(min_value == UINT64_MAX);
    CHECK(max_value == UINT64_MAX);

    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(
        CTX,
        blind, &value,
        message, &m_len,
        nonce_3,
        &min_value, &max_value,
        &pc,
        vector_3, sizeof(vector_3),
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ));
    CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blind, blind_3, 32) == 0);
    CHECK(value == UINT64_MAX);
    CHECK(min_value == UINT64_MAX);
    CHECK(max_value == UINT64_MAX);
    CHECK(m_len == 0);
}
}

static void print_vector_helper(unsigned char *buf, size_t buf_len) {
    size_t j;
    printf("    ");
    for (j = 0; j < buf_len; j++) {
        printf("0x%02x", buf[j]);
        if (j == buf_len-1) {
            printf(",\n");
        } else if ((j+1) % 16 != 0) {
            printf(", ");
        } else {
            printf(",\n");
            printf("    ");
        }
    }
    printf("};\n");
}

static void print_vector(int i, unsigned char *proof, size_t p_len, rustsecp256k1zkp_v0_8_1_pedersen_commitment *commit) {
    unsigned char commit_output[33];

    printf("unsigned char vector_%d[] = {\n", i);
    print_vector_helper(proof, p_len);

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commitment_serialize(CTX, commit_output, commit));
    printf("unsigned char commit_%d[] = {\n", i);
    print_vector_helper(commit_output, sizeof(commit_output));
}


/* Use same nonce and blinding value for all "reproducible" test vectors */
static unsigned char vector_blind[] = {
    0x48, 0x26, 0xad, 0x41, 0x37, 0x4c, 0x25, 0x62, 0x52, 0x14, 0x78, 0x82, 0x89, 0x9c, 0x86, 0x27,
    0xa1, 0x19, 0xf6, 0xe1, 0xfa, 0x44, 0xe4, 0x29, 0x08, 0xa7, 0xb3, 0x45, 0xad, 0x35, 0xb2, 0xd9,
};
static unsigned char vector_nonce[] = {
    0xc8, 0x5c, 0x7e, 0x6c, 0xa1, 0xfa, 0x11, 0x35, 0xc7, 0x45, 0x24, 0x8a, 0xb5, 0x28, 0x6d, 0x1a,
    0x88, 0x00, 0xff, 0xca, 0x96, 0x0f, 0xc7, 0x77, 0xa5, 0x96, 0x7a, 0x5e, 0xf8, 0x88, 0x2d, 0xd4,
};

/* Maximum length of a message that can be embedded into a rangeproof */
static void test_rangeproof_fixed_vectors_reproducible_helper(unsigned char *vector, size_t vector_len, unsigned char *commit, uint64_t *value_r, uint64_t *min_value_r, uint64_t *max_value_r, unsigned char *message_r, size_t *m_len_r) {
    rustsecp256k1zkp_v0_8_1_pedersen_commitment pc;
    unsigned char blind_r[32];

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commitment_parse(CTX, &pc, commit));
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_verify(
        CTX,
        min_value_r, max_value_r,
        &pc,
        vector, vector_len,
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ));

    *m_len_r = SECP256K1_RANGEPROOF_MAX_MESSAGE_LEN;
    CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_rewind(
        CTX,
        blind_r, value_r,
        message_r, m_len_r,
        vector_nonce,
        min_value_r, max_value_r,
        &pc,
        vector, vector_len,
        NULL, 0,
        rustsecp256k1zkp_v0_8_1_generator_h
    ));
    CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(blind_r, vector_blind, sizeof(vector_blind)) == 0);
}

static void test_rangeproof_fixed_vectors_reproducible(void) {
    uint64_t value_r;
    uint64_t min_value_r;
    uint64_t max_value_r;
    unsigned char message[SECP256K1_RANGEPROOF_MAX_MESSAGE_LEN], message_r[SECP256K1_RANGEPROOF_MAX_MESSAGE_LEN];
    size_t m_len_r;
    memset(message, 0xFF, sizeof(message));

    /* Test maximum values for value, min_bits, m_len and exp */
    {
        uint64_t value = UINT64_MAX;
        uint64_t min_value = 0;
        size_t m_len = sizeof(message); /* maximum message length */

        int min_bits = 64;
        int exp  = 18;
        unsigned char proof[5126];
        size_t p_len = sizeof(proof);
        rustsecp256k1zkp_v0_8_1_pedersen_commitment pc;

        unsigned char vector_0[] = {
            ,
        };
        unsigned char commit_0[] = {
            0x09, 0x9e, 0x56, 0x8d, 0x5b, 0x9d, 0x2a, 0xd6, 0x1f, 0xe0, 0x81, 0x21, 0xcc, 0x15, 0xb3, 0x66,
            0x6d, 0xb4, 0xbb, 0xac, 0xdd, 0x28, 0x08, 0xab, 0x21, 0x6e, 0x35, 0xac, 0xa7, 0xe0, 0x0a, 0xa8,
            0xef,
        };

        CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &pc, vector_blind, value, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &p_len, min_value, &pc, vector_blind, vector_nonce, exp, min_bits, value, message, m_len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(p_len <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, value, min_bits));
        CHECK(p_len == sizeof(proof));
        /* Uncomment the next line to print the test vector */
        /* print_vector(0, proof, p_len, &pc); */
        CHECK(p_len == sizeof(vector_0));
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(proof, vector_0, p_len) == 0);

        test_rangeproof_fixed_vectors_reproducible_helper(vector_0, sizeof(vector_0), commit_0, &value_r, &min_value_r, &max_value_r, message_r, &m_len_r);
        CHECK(value_r == value);
        CHECK(m_len_r == m_len);
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(message_r, message, m_len_r) == 0);
        CHECK(min_value_r == min_value);
        CHECK(max_value_r == UINT64_MAX);
        memset(message_r, 0, sizeof(message_r));
    }

    /* Test min_bits = 3 */
    {
        uint64_t value = 13;
        size_t m_len = 128; /* maximum message length with min_bits = 3 */

        uint64_t min_value = 1;
        int min_bits = 3;
        int exp  = 1;
        unsigned char proof[267];
        size_t p_len = sizeof(proof);
        rustsecp256k1zkp_v0_8_1_pedersen_commitment pc;

        unsigned char vector_1[] = {
            ,
        };
        unsigned char commit_1[] = {
            0x09, 0xe5, 0xb3, 0x27, 0x82, 0x88, 0xeb, 0x21, 0xcd, 0xb2, 0x56, 0x37, 0x61, 0x84, 0xce, 0xc1,
            0x66, 0x16, 0x2e, 0x44, 0xc8, 0x65, 0x8e, 0xe6, 0x3a, 0x1a, 0x57, 0x2c, 0xb9, 0x6c, 0x07, 0x85,
            0xf0,
        };
        CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &pc, vector_blind, value, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &p_len, min_value, &pc, vector_blind, vector_nonce, exp, min_bits, value, message, m_len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(p_len <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, value, min_bits));
        CHECK(p_len == sizeof(proof));
        /* Uncomment the next line to print the test vector */
        /* print_vector(1, proof, p_len, &pc); */
        CHECK(p_len == sizeof(vector_1));
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(proof, vector_1, p_len) == 0);

        test_rangeproof_fixed_vectors_reproducible_helper(vector_1, sizeof(vector_1), commit_1, &value_r, &min_value_r, &max_value_r, message_r, &m_len_r);
        CHECK(value_r == value);
        CHECK(m_len_r == m_len);
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(message_r, message, m_len_r) == 0);
        CHECK(min_value_r == 3);
        CHECK(max_value_r == 73);
        memset(message_r, 0, sizeof(message_r));
    }

    /* Test large min_value */
    {
        uint64_t value = INT64_MAX;
        size_t m_len = 0; /* maximum message length with min_bits = 3 */

        /* Uncomment this to recreate test vector */
        uint64_t min_value = INT64_MAX-1;
        int min_bits = 1;
        int exp  = 0;
        unsigned char proof[106];
        size_t p_len = sizeof(proof);
        rustsecp256k1zkp_v0_8_1_pedersen_commitment pc;

        unsigned char vector_2[] = {
            0x60, 0x00, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x81, 0xd8, 0x21, 0x12, 0x4d, 0xa4,
            0x84, 0xdd, 0x2c, 0xd1, 0x04, 0xe7, 0x08, 0x9a, 0xd3, 0x6f, 0xa5, 0xd8, 0xfc, 0x52, 0x4c, 0xba,
            0xf0, 0x83, 0xeb, 0x76, 0x9f, 0x1c, 0x03, 0xe3, 0xcf, 0x23, 0x1e, 0x40, 0x18, 0xc6, 0x6d, 0xf9,
            0x25, 0x56, 0x80, 0x3c, 0x83, 0xdd, 0x58, 0x36, 0x43, 0xe3, 0x56, 0xa0, 0xb7, 0xf0, 0x0e, 0xf9,
            0xe2, 0x8b, 0x82, 0x5a, 0x77, 0xa7, 0xbe, 0x36, 0x98, 0x10, 0x99, 0x2e, 0xaa, 0x21, 0x24, 0xe6,
            0x78, 0xa8, 0xcc, 0xc7, 0x06, 0x1c, 0x06, 0xb0, 0x03, 0x87, 0x86, 0x89, 0xce, 0x85, 0x88, 0xea,
            0xa1, 0x9d, 0x4d, 0xfd, 0x8d, 0x65, 0xbd, 0xa9, 0xd0, 0x0f,
        };
        unsigned char commit_2[] = {
            0x09, 0x2a, 0x74, 0xa1, 0x9c, 0xee, 0xcb, 0x6a, 0xd1, 0xa7, 0x97, 0xbe, 0x97, 0xe7, 0xb6, 0x37,
            0x90, 0x96, 0xc2, 0x5a, 0xe5, 0xfc, 0xed, 0x91, 0xff, 0x4c, 0x67, 0x07, 0x96, 0x1d, 0x2a, 0xb3,
            0x70,
        };

        CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &pc, vector_blind, value, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(rustsecp256k1zkp_v0_8_1_rangeproof_sign(CTX, proof, &p_len, min_value, &pc, vector_blind, vector_nonce, exp, min_bits, value, message, m_len, NULL, 0, rustsecp256k1zkp_v0_8_1_generator_h));
        CHECK(p_len <= rustsecp256k1zkp_v0_8_1_rangeproof_max_size(CTX, value, min_bits));
        CHECK(p_len == sizeof(proof));
        /* Uncomment the next line to print the test vector */
        /* print_vector(2, proof, p_len, &pc); */
        CHECK(p_len == sizeof(vector_2));
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(proof, vector_2, p_len) == 0);

        test_rangeproof_fixed_vectors_reproducible_helper(vector_2, sizeof(vector_2), commit_2, &value_r, &min_value_r, &max_value_r, message_r, &m_len_r);
        CHECK(value_r == value);
        CHECK(m_len_r == m_len);
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(message_r, message, m_len_r) == 0);
        CHECK(min_value_r == INT64_MAX-1);
        CHECK(max_value_r == INT64_MAX);
        memset(message_r, 0, sizeof(message_r));
    }
}

static void run_rangeproof_tests(void) {
    int i;
    test_api();

    test_single_value_proof(0);
    test_single_value_proof(12345678);
    test_single_value_proof(UINT64_MAX);

    test_rangeproof_fixed_vectors();
    test_rangeproof_fixed_vectors_reproducible();
    for (i = 0; i < COUNT / 2 + 1; i++) {
        test_borromean();
    }
    test_rangeproof();
    test_rangeproof_null_blinder();
    test_multiple_generators();
}

#endif
