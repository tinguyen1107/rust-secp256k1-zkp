...
static void test_exhaustive_ellswift(const rustsecp256k1zkp_v0_8_1_context *ctx, const rustsecp256k1zkp_v0_8_1_ge *group) {
    int i;
    ...
    for (i = 1; i < EXHAUSTIVE_TEST_ORDER; i++) {
        ...
        rustsecp256k1zkp_v0_8_1_pubkey pub_decoded;
        rustsecp256k1zkp_v0_8_1_ge ge_decoded;

        /* Construct ellswift pubkey from exhaustive loop scalar i. */
        rustsecp256k1zkp_v0_8_1_scalar_set_int(&scalar_i, i);
        rustsecp256k1zkp_v0_8_1_scalar_get_b32(sec32, &scalar_i);
        CHECK(rustsecp256k1zkp_v0_8_1_ellswift_create(ctx, ell64, sec32, NULL));

        /* Decode ellswift pubkey and check that it matches the precomputed group element. */
        rustsecp256k1zkp_v0_8_1_ellswift_decode(ctx, &pub_decoded, ell64);
        rustsecp256k1zkp_v0_8_1_pubkey_load(ctx, &ge_decoded, &pub_decoded);
