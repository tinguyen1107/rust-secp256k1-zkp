...
typedef struct {
    ...
    rustsecp256k1zkp_v0_8_1_pubkey point[256];
...
static void bench_ellswift_setup(void *arg) {
    ...
    for (i = 0; i < 256; ++i) {
        int j;
        CHECK(rustsecp256k1zkp_v0_8_1_ellswift_decode(data->ctx, &data->point[i], data->rnd64));
        for (j = 0; j < 64; ++j) {
            data->rnd64[j] += 1;
        }
    }
    CHECK(rustsecp256k1zkp_v0_8_1_ellswift_encode(data->ctx, data->rnd64, &data->point[255], init + 16));
}
...
static void bench_ellswift_encode(void *arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        CHECK(rustsecp256k1zkp_v0_8_1_ellswift_encode(data->ctx, data->rnd64, &data->point[i & 255], data->rnd64 + 16));
    }
}
...
static void bench_ellswift_create(void *arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(rustsecp256k1zkp_v0_8_1_ellswift_create(data->ctx, buf, data->rnd64, data->rnd64 + 32));
...
static void bench_ellswift_decode(void *arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        CHECK(rustsecp256k1zkp_v0_8_1_ellswift_decode(data->ctx, &out, data->rnd64) == 1);
        len = 33;
        CHECK(rustsecp256k1zkp_v0_8_1_ec_pubkey_serialize(data->ctx, data->rnd64 + (i % 32), &len, &out, SECP256K1_EC_COMPRESSED));
    }
}
...
static void bench_ellswift_xdh(void *arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        int party = i & 1;
        CHECK(rustsecp256k1zkp_v0_8_1_ellswift_xdh(data->ctx,
                                     data->rnd64 + (i % 33),
                                     data->rnd64,
                                     data->rnd64,
                                     data->rnd64 + ((i + 16) % 33),
                                     party,
                                     rustsecp256k1zkp_v0_8_1_ellswift_xdh_hash_function_bip324,
                                     NULL) == 1);
    }
}
