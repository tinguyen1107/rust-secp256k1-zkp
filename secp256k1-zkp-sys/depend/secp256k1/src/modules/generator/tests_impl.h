...
static void test_generator_api(void) {
    ...
    rustsecp256k1zkp_v0_8_1_generator gen;
    int32_t ecount = 0;
    ...
    rustsecp256k1zkp_v0_8_1_testrand256(key);
    rustsecp256k1zkp_v0_8_1_testrand256(blind);

    CHECK(rustsecp256k1zkp_v0_8_1_generator_generate(CTX, &gen, key) == 1);
    CHECK(ecount == 0);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_generate(CTX, NULL, key) == 0);
    CHECK(ecount == 1);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_generate(CTX, &gen, NULL) == 0);
    CHECK(ecount == 2);

    CHECK(rustsecp256k1zkp_v0_8_1_generator_generate_blinded(CTX, &gen, key, blind) == 1);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_generate_blinded(STATIC_CTX, &gen, key, blind) == 0);
    CHECK(ecount == 3);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_generate_blinded(CTX, NULL, key, blind) == 0);
    CHECK(ecount == 4);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_generate_blinded(CTX, &gen, NULL, blind) == 0);
    CHECK(ecount == 5);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_generate_blinded(CTX, &gen, key, NULL) == 0);
    CHECK(ecount == 6);

    CHECK(rustsecp256k1zkp_v0_8_1_generator_serialize(CTX, sergen, &gen) == 1);
    CHECK(ecount == 6);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_serialize(CTX, NULL, &gen) == 0);
    CHECK(ecount == 7);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_serialize(CTX, sergen, NULL) == 0);
    CHECK(ecount == 8);

    CHECK(rustsecp256k1zkp_v0_8_1_generator_serialize(CTX, sergen, &gen) == 1);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_parse(CTX, &gen, sergen) == 1);
    CHECK(ecount == 8);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_parse(CTX, NULL, sergen) == 0);
    CHECK(ecount == 9);
    CHECK(rustsecp256k1zkp_v0_8_1_generator_parse(CTX, &gen, NULL) == 0);
    CHECK(ecount == 10);
...
static void test_shallue_van_de_woestijne(void) {
    ...
    static const rustsecp256k1zkp_v0_8_1_ge_storage results[32] = {
    ...
    rustsecp256k1zkp_v0_8_1_ge_storage ges;
    int i, s;
    for (i = 1; i <= 16; i++) {
        rustsecp256k1zkp_v0_8_1_fe_set_int(&fe, i);

        for (s = 0; s < 2; s++) {
            if (s) {
                rustsecp256k1zkp_v0_8_1_fe_negate(&fe, &fe, 1);
                rustsecp256k1zkp_v0_8_1_fe_normalize(&fe);
            }
            shallue_van_de_woestijne(&ge, &fe);
            rustsecp256k1zkp_v0_8_1_ge_to_storage(&ges, &ge);

            CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(&ges, &results[i * 2 + s - 2], sizeof(rustsecp256k1zkp_v0_8_1_ge_storage)) == 0);
        }
    }
}
...
static void test_generator_generate(void) {
    ...
    rustsecp256k1zkp_v0_8_1_scalar_set_b32(&sc, s, NULL);
    for (i = 1; i <= 32; i++) {
        memset(v, 0, 31);
        v[31] = i;
        CHECK(rustsecp256k1zkp_v0_8_1_generator_generate_blinded(CTX, &gen, v, s));
        rustsecp256k1zkp_v0_8_1_generator_load(&ge, &gen);
        rustsecp256k1zkp_v0_8_1_ge_to_storage(&ges, &ge);
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(&ges, &results[i - 1], sizeof(rustsecp256k1zkp_v0_8_1_ge_storage)) == 0);
        CHECK(rustsecp256k1zkp_v0_8_1_generator_generate(CTX, &gen, v));
        rustsecp256k1zkp_v0_8_1_generator_load(&ge, &gen);
        rustsecp256k1zkp_v0_8_1_ge_to_storage(&ges, &ge);
        CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(&ges, &results[i - 1], sizeof(rustsecp256k1zkp_v0_8_1_ge_storage)) == 0);
    }
...
static void test_generator_fixed_vector(void) {
    ...
    rustsecp256k1zkp_v0_8_1_generator parse;

    CHECK(rustsecp256k1zkp_v0_8_1_generator_parse(CTX, &parse, two_g));
    CHECK(rustsecp256k1zkp_v0_8_1_generator_serialize(CTX, result, &parse));
    CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(two_g, result, 33) == 0);

    result[0] = 0x0a;
    CHECK(rustsecp256k1zkp_v0_8_1_generator_parse(CTX, &parse, result));
    result[0] = 0x08;
    CHECK(!rustsecp256k1zkp_v0_8_1_generator_parse(CTX, &parse, result));
}

static void test_pedersen_api(void) {
    rustsecp256k1zkp_v0_8_1_pedersen_commitment commit;
    const rustsecp256k1zkp_v0_8_1_pedersen_commitment *commit_ptr = &commit;
    unsigned char blind[32];
    unsigned char blind_out[32];
    const unsigned char *blind_ptr = blind;
    unsigned char *blind_out_ptr = blind_out;
    uint64_t val = rustsecp256k1zkp_v0_8_1_testrand32();
    int32_t ecount = 0;
    ...
    rustsecp256k1zkp_v0_8_1_testrand256(blind);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, val, rustsecp256k1zkp_v0_8_1_generator_h) != 0);
    CHECK(ecount == 0);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(STATIC_CTX, &commit, blind, val, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(ecount == 1);

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, NULL, blind, val, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(ecount == 2);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, NULL, val, rustsecp256k1zkp_v0_8_1_generator_h) == 0);
    CHECK(ecount == 3);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, val, NULL) == 0);
    CHECK(ecount == 4);

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_sum(CTX, blind_out, &blind_ptr, 1, 1) != 0);
    CHECK(ecount == 4);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_sum(CTX, NULL, &blind_ptr, 1, 1) == 0);
    CHECK(ecount == 5);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_sum(CTX, blind_out, NULL, 1, 1) == 0);
    CHECK(ecount == 6);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_sum(CTX, blind_out, &blind_ptr, 0, 1) == 0);
    CHECK(ecount == 7);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_sum(CTX, blind_out, &blind_ptr, 0, 0) != 0);
    CHECK(ecount == 7);

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commit, blind, val, rustsecp256k1zkp_v0_8_1_generator_h) != 0);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, &commit_ptr, 1, &commit_ptr, 1) != 0);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, NULL, 0, &commit_ptr, 1) == 0);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, &commit_ptr, 1, NULL, 0) == 0);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, NULL, 0, NULL, 0) != 0);
    CHECK(ecount == 7);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, NULL, 1, &commit_ptr, 1) == 0);
    CHECK(ecount == 8);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, &commit_ptr, 1, NULL, 1) == 0);
    CHECK(ecount == 9);

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 1, 0) != 0);
    CHECK(ecount == 9);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 1, 1) == 0);
    CHECK(ecount == 10);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, &blind_out_ptr, 0, 0) == 0);
    CHECK(ecount == 11);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_generator_blind_sum(CTX, NULL, &blind_ptr, &blind_out_ptr, 1, 0) == 0);
    CHECK(ecount == 12);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_generator_blind_sum(CTX, &val, NULL, &blind_out_ptr, 1, 0) == 0);
    CHECK(ecount == 13);
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_generator_blind_sum(CTX, &val, &blind_ptr, NULL, 1, 0) == 0);
    CHECK(ecount == 14);
...
static void test_pedersen(void) {
    rustsecp256k1zkp_v0_8_1_pedersen_commitment commits[19];
    const rustsecp256k1zkp_v0_8_1_pedersen_commitment *cptr[19];
    ...
    int outputs;
    int total;
    inputs = (rustsecp256k1zkp_v0_8_1_testrand32() & 7) + 1;
    outputs = (rustsecp256k1zkp_v0_8_1_testrand32() & 7) + 2;
    ...
    totalv = 0;
    for (i = 0; i < inputs; i++) {
        values[i] = rustsecp256k1zkp_v0_8_1_testrandi64(0, INT64_MAX - totalv);
        totalv += values[i];
    }
    for (i = 0; i < outputs - 1; i++) {
        values[i + inputs] = rustsecp256k1zkp_v0_8_1_testrandi64(0, totalv);
    ...
    for (i = 0; i < total - 1; i++) {
        ...
        rustsecp256k1zkp_v0_8_1_scalar_get_b32(&blinds[i * 32], &s);
    }
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_blind_sum(CTX, &blinds[(total - 1) * 32], bptr, total - 1, inputs));
    for (i = 0; i < total; i++) {
        CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commit(CTX, &commits[i], &blinds[i * 32], values[i], rustsecp256k1zkp_v0_8_1_generator_h));
    }
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, cptr, inputs, &cptr[inputs], outputs));
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, &cptr[inputs], outputs, cptr, inputs));
    if (inputs > 0 && values[0] > 0) {
        CHECK(!rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, cptr, inputs - 1, &cptr[inputs], outputs));
    }
    random_scalar_order(&s);
    for (i = 0; i < 4; i++) {
        rustsecp256k1zkp_v0_8_1_scalar_get_b32(&blinds[i * 32], &s);
    ...
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, &cptr[0], 1, &cptr[0], 1));
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_verify_tally(CTX, &cptr[1], 1, &cptr[1], 1));
}
...
static void test_pedersen_commitment_fixed_vector(void) {
    ...
    unsigned char result[33];
    rustsecp256k1zkp_v0_8_1_pedersen_commitment parse;

    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commitment_parse(CTX, &parse, two_g));
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commitment_serialize(CTX, result, &parse));
    CHECK(rustsecp256k1zkp_v0_8_1_memcmp_var(two_g, result, 33) == 0);

    result[0] = 0x08;
    CHECK(rustsecp256k1zkp_v0_8_1_pedersen_commitment_parse(CTX, &parse, result));
    result[0] = 0x0c;
    CHECK(!rustsecp256k1zkp_v0_8_1_pedersen_commitment_parse(CTX, &parse, result));
}
