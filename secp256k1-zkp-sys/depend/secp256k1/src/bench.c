...
static void bench_verify(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(rustsecp256k1zkp_v0_8_1_ec_pubkey_parse(data->ctx, &pubkey, data->pubkey, data->pubkeylen) == 1);
        CHECK(rustsecp256k1zkp_v0_8_1_ecdsa_signature_parse_der(data->ctx, &sig, data->sig, data->siglen) == 1);
        CHECK(rustsecp256k1zkp_v0_8_1_ecdsa_verify(data->ctx, &sig, data->msg, &pubkey) == (i == 0));
        data->sig[data->siglen - 1] ^= (i & 0xFF);
        data->sig[data->siglen - 2] ^= ((i >> 8) & 0xFF);
        data->sig[data->siglen - 3] ^= ((i >> 16) & 0xFF);
    }
...
static void bench_sign_run(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(rustsecp256k1zkp_v0_8_1_ecdsa_sign(data->ctx, &signature, data->msg, data->key, NULL, NULL));
        CHECK(rustsecp256k1zkp_v0_8_1_ecdsa_signature_serialize_der(data->ctx, sig, &siglen, &signature));
        for (j = 0; j < 32; j++) {
            data->msg[j] = sig[j];
            data->key[j] = sig[j + 32];
        }
...
static void bench_keygen_run(void *arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        size_t len = 33;
        rustsecp256k1zkp_v0_8_1_pubkey pubkey;
        CHECK(rustsecp256k1zkp_v0_8_1_ec_pubkey_create(data->ctx, &pubkey, data->key));
        CHECK(rustsecp256k1zkp_v0_8_1_ec_pubkey_serialize(data->ctx, pub33, &len, &pubkey, SECP256K1_EC_COMPRESSED));
...
/* Check if the user tries to benchmark optional module without building it */
    ...
    data.siglen = 72;
    CHECK(rustsecp256k1zkp_v0_8_1_ecdsa_sign(data.ctx, &sig, data.msg, data.key, NULL, NULL));
    CHECK(rustsecp256k1zkp_v0_8_1_ecdsa_signature_serialize_der(data.ctx, data.sig, &data.siglen, &sig));
    CHECK(rustsecp256k1zkp_v0_8_1_ec_pubkey_create(data.ctx, &pubkey, data.key));
    data.pubkeylen = 33;
    CHECK(rustsecp256k1zkp_v0_8_1_ec_pubkey_serialize(data.ctx, data.pubkey, &data.pubkeylen, &pubkey, SECP256K1_EC_COMPRESSED) == 1);
