...
static void bench_generator_generate(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(rustsecp256k1zkp_v0_8_1_generator_generate(data->ctx, &gen, data->key));
        data->key[i & 31]++;
    }
}
...
static void bench_generator_generate_blinded(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1zkp_v0_8_1_generator gen;
        CHECK(rustsecp256k1zkp_v0_8_1_generator_generate_blinded(data->ctx, &gen, data->key, data->blind));
        data->key[1 + (i & 30)]++;
        data->blind[1 + (i & 30)]++;
    }
}
