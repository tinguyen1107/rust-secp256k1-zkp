...
This is implemented in `rustsecp256k1zkp_v0_8_1_ellswift_xswiftec_frac_var` (which decodes to an x-coordinate represented as a fraction), and
in `rustsecp256k1zkp_v0_8_1_ellswift_xswiftec_var` (which outputs the actual x-coordinate).
...
This is implemented in `rustsecp256k1zkp_v0_8_1_ellswift_xelligatorswift_var`.
...
This is implemented in `rustsecp256k1zkp_v0_8_1_ellswift_xswiftec_inv_var`.
...
This is implemented in `rustsecp256k1zkp_v0_8_1_ellswift_swiftec_var`. The used $sign(x)$ function is the parity of $x$ when represented as in integer in $[0,q).$

The corresponding encoder would invoke the x-only one, but negating the output $t$ if $sign(t) \neq sign(y).$

This is implemented in `rustsecp256k1zkp_v0_8_1_ellswift_elligatorswift_var`.
